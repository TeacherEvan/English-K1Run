...
### Mobile / Native Build (Capacitor) — `android/` & `ios/`

- Native Android/iOS project shells generated by Capacitor 8.5.0 and committed to the repo.
- Created with `npx cap add android` / `npx cap add ios`; the web `dist/` output is synced in via `npx cap sync`.
- The web source of truth stays under `src/`; the native shells only wrap the built PWA (no rewrite of the web app).
- Capacitor config: `capacitor.config.ts` (appId `com.teacherevan.kindergartenrace`, appName `Kindergarten Race`, webDir `dist`).
