...
export const preloadResources = async (
  ...
  const updateProgress = () => {
    globalPreloadProgress.percentage = Math.round(
      ((globalPreloadProgress.loaded + globalPreloadProgress.failed) / globalPreloadProgress.total) * 100
    )
...
export const preloadCriticalResources = async (
  ...
  const criticalResources: ResourceMetadata[] = [
    ...
    { url: '/backgrounds/mountain-sunrise.jpg', type: 'image', priority: 'high' },
    ...
    { url: '/sounds/wrong.wav', type: 'audio', priority: 'medium' },
  ...
  const resourcesToLoad = criticalResources.filter(r => priorities.includes(r.priority))
  
  return preloadResources(resourcesToLoad, (progress) => {
    if (import.meta.env.DEV) {
      console.log(`[ResourcePreloader] Loading critical resources: ${progress.percentage}%`)
    }
  })
}
