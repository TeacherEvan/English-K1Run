...
- Coordinates are percentage-based only. X ranges 5–95% with `LANE_BOUNDS` in [src/lib/constants/game-config.ts](../src/lib/constants/game-config.ts). Never use pixel positions for gameplay objects.
...
- Frequently re-rendered components use `memo()` (example: [src/components/FallingObject.tsx](../src/components/FallingObject.tsx)).
- Game menu layout keeps header, grid (`flex-1`), and footer as siblings; avoid wrappers that break layout in [src/components/GameMenu.tsx](../src/components/GameMenu.tsx).
